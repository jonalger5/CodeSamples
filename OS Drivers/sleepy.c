	     ...
	     loff_t *f_pos)
{
  ...
  //Checking for a 4 byte input
  if(count != 4){
    mutex_unlock(&dev->sleepy_mutex);
    return -EINVAL;
  }
  
  //Turning input into an int
  //printk("%s\n", buf);
  dev->data = buf;
  schedtime = *(int*)dev->data;
  //printk("%d\n", schedtime);
  //Checking if value is positive
  if(schedtime >= 0){
    //j = jiffies;
    timeout = schedtime * HZ;
    //printk( KERN_DEBUG "Before wait\n");
    waitret = wait_event_interruptible_timeout(dev->wq, 0, timeout);
    //printk( KERN_DEBUG "After wait\n");
    //Time has fully elapsed and returns 0
    if(waitret == 0)
      retval = 0;
    //Time didn't finish and returns the amount of time left
    else {
      //printk( KERN_DEBUG "HERE\n");
      diff = (long)waitret - (long)j;
      //printk("%ld\n", diff);
      retval = diff / HZ;
    }
  }
  //Return EINVAL if the number was negative
  else{
    mutex_unlock(&dev->sleepy_mutex);
    return -EINVAL;
  }
